#!/bin/sh
# This script is copied from neutron and adapted for starfish.
set -eu

usage () {
    echo "Usage: $0 [OPTION]..."
    echo "Run starfish's coding check(s)"
    echo ""
    echo "  -Y, --pylint [<basecommit>] Run pylint check on the entire starfish module or just files changed in basecommit (e.g. HEAD~1)"
